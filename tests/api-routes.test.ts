...
describe('prototype API routes', () => {
  ...
  it('preserves empty arrays, null values, genuine zero, stale freshness, and provenance', async () => {
    ...
    const siteContentBody = await siteContent.json() as { data: Record<string, unknown> & { carbonPlan: { progressPercent: null } } };
    expect(siteContentBody.data).not.toHaveProperty('start');
