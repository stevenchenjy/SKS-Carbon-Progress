...
describe('mock provider contracts', () => {
  ...
  it('returns all five roadmap areas with bounded example progress', async () => {
    ...
    expect(areas.every((area) => area.progress.percent >= 0 && area.progress.percent <= 100)).toBe(true);
  });
});
